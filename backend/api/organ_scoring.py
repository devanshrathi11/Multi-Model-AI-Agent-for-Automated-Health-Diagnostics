        ...
        if ref_range:
            try:
                ...
                if "-" in ref_range:
                    ...
                    range_span = max_val - min_val if max_val != min_val else max_val
                    if compare_val < min_val:
                        deviation = (min_val - compare_val) / (range_span if range_span > 0 else min_val) * 100
                        if deviation > 30:
                            status = "critical"
                        else:
                            status = "low"
                    elif compare_val > max_val:
                        deviation = (compare_val - max_val) / (range_span if range_span > 0 else max_val) * 100
                        if deviation > 30:
                            status = "critical"
                        else:
                            status = "high"
                    ...
                    if compare_val < min_val * 0.5:
                        status = "critical"
                    elif compare_val < min_val:
                        status = "low"
                    else:
                        status = "normal"
            except Exception:
